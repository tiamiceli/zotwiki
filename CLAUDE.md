...
All planned work through BUG-1 is complete. The implemented subcommands are `ingest`, `compile`, `audit`, `ask`, and `sync`. 53 requirements are green (REQ-001–REQ-053); the suite baseline is **387 passed, 1 failed** on macOS — the one failure (`test_req_019`) is a known, accepted macOS case-collision limitation, not a regression (on case-sensitive Linux it passes, so CI shows all green). BUG-1 (sync duplicate pages) is fixed (Ruling 6, `docs/plan-bug1.md`). The `zw` terminal wrapper (`scripts/zw`, contract §11, Rulings 7 & 8, REQ-049–053) lets the operator drive zotwiki from a plain terminal: `ZOTWIKI_VAULT` is the Obsidian Library folder and `ZOTWIKI_COLLECTION` the collection, so each collection becomes its own wiki subfolder. Known-open item: BUG-2 (occasional LLM schema errors / nested-session char-0 failures — the structured-output fix is **authorized but not yet built**). It is planned (Ruling 9, `docs/plan-bug2.md`): `ClaudeCodeLLMClient` moves to `claude --print --output-format json --json-schema <schema> --exclude-dynamic-system-prompt-sections`, reads `structured_output`, strips `CLAUDECODE`/`CLAUDE_CODE_*`, and fails closed to a verbatim artifact. Contract §5.1/§5.6/§9.3/§9.4/§9.5 and REQ-039 (revised) + REQ-054/REQ-055 are written; **next is the tester red gate (`tests/test_m6_llm_client.py`) then the coder (`llm.py`/`ask.py`/`cli.py`)**. The §7.1 OAuth precondition is resolved (validated 2026-06-22).
